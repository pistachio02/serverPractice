...
module.exports = (req, res) => {    
    ...
    Users
        ...
        .then((data) => {
            if(!data) {
                return res.status(401).json({ "message": "Invalid accessToken!" });
                // 만약 등록된 이메일(유저)이 아닌경우, 유효하지 않은 토큰이야 라고 응답으로 안내해준다.
            }
            return res
                    .clearCookie('accessToken')
                    .status(205)
                    .json({ message: 'Successfully Logged Out!' });
            // 만약 요청 쿠키에 엑세스토큰도 잘 담겨져 왔고, 그 토큰을 해독했을때 나오는 유저의 정보로 데이터베이스를 조회했을때 기존에 등록된 유저라는게 확인이 된다면,
            // 쿠키에 담긴 엑세스토큰을 삭제해주고, 상태코드 205 reset content 를 응답에 담아 보내주고, 응답 객체의 메세지에 로그아웃 잘 됐다 라고 안내해준다.
        })
};